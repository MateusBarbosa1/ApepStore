...
module.exports.renderPageProdutoSingle = async function (app, req, res) {
  ...
  if (token) {
    ...
    if (usuario.length !== 0) {
      ...
      const carrinhoUsuario = usuario[0].carrinho;
      let produtoCarrinho = {};

      for (let i = 0; i < carrinhoUsuario.length; i++) {
        if (carrinhoUsuario[i].id == idProduto) {
          produtoCarrinho = carrinhoUsuario[i];
        }
      }

      return res.render("produtos/produto-single", {
        nome: firstName,
        produto,
  ...
  // Usuário não logado
  res.render("produtos/produto-single", {
    nome: false,
    produto,
    produtoCarrinho: false,
    path: req.path,
  });
};
