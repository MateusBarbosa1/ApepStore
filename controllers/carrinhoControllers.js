  ...
  if (token) {
    ...
    if (usuario.length !== 0) {
      ...
      for (let i = 0; i < carrinho.length; i++) {
        const produtoIdCarrinho = await produtosModel.getProdutoID(
          carrinho[i].id
        );

        if (produtoIdCarrinho != false) {
          ...
          // carrinho ta com um produto deletado
          await usuariosModel.removeProdutoCarrinho(
            tokenDecoded.id,
            carrinho[i].id
          );
        }
