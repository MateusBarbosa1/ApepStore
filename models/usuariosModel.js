...
async function getCarrinho(id_usuario) {
  try {
    const produtosCarrinho = await prisma.usuarios.findMany({
      where: { id: id_usuario },
    });
...
async function updateCarrinho(id_usuario, id_produto, data, type) {
  try {
    const carrinhoAntigo = await prisma.usuarios.findMany({
      where: { id: id_usuario },
    });
    ...
    for (let i = 0; i < carrinhoAntigoArray.length; i++) {
      if (carrinhoAntigoArray[i].id == id_produto) {
        // Produto Para Atualizar
        if (type == "qtd") {
          novoCarrinho.push({
            id: id_produto,
            cor: carrinhoAntigoArray[i].cor,
            qtd: data,
            size: carrinhoAntigoArray[i].size,
          });
        }
        if (type == "all") {
          novoCarrinho.push({
